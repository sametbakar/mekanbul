...
function AddComment() {
  ...
  const onSubmit = (evt) => {
    evt.preventDefault();
    if (evt.target.elements.author.value && evt.target.elements.rating.value && evt.target.elements.text.value) {
      let newComment = {
        author: evt.target.elements.author.value,
        rating: evt.target.elements.rating.value,
        text: evt.target.elements.text.value
      }
      VenueDataService.addComment(id, newComment).then(function (response) {
